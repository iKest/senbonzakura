...
local function createText()
    local memory = display.newText('00 00.00 000',10,0, 'Helvetica', 24);
...
function _M.labelUpdater(event)
    ...
    _M.text.text = 'FPS:' .. tostring(mFloor( 1000 / (curTime - prevTime))) .. ' Texture Memory:' ..
            tostring(mFloor(sGetInfo('textureMemoryUsed') * 0.0001) * 0.01) .. ' Lua Memory:' ..
